                        ...
                        <div class="flex flex-col justify-between">
                            ...
                            {#if item.war && ((item.war as APIClanWar).state === "preparation" || (item.war as APIClanWar).state === "inWar" || (item.war as APIClanWar).state === "warEnded")}
                                <div class="mx-2 mb-2 flex flex-col">
                                    ...
                                    <div
                                        class="border-background bg-foreground/10 inset-shadow-foreground flex w-full items-center space-x-3 rounded-xl border-1 p-4 inset-shadow-sm"
                                    >
                                        <img
                                            class="size-11"
                                            src={(item.war as APIClanWar).opponent.badgeUrls.medium}
                                            alt={(item.war as APIClanWar).opponent.name}
                                            loading="lazy"
                                            decoding="async"
                                            fetchpriority="low"
                                        />
                                ...
                                <div class="mx-2 mb-3 flex justify-end gap-2">
                                    ...
                                    <DropdownMenu.Root>
                                        ...
                                        <DropdownMenu.Content
                                            class="border-background/50 bg-background/95 w-96 overflow-hidden rounded-xl border-1 p-0 shadow-xl backdrop-blur md:w-[34rem]"
                                        >
                                            <DropdownMenu.Label class="bg-foreground/5 px-4 py-3 text-sm font-semibold"
                                                >War details</DropdownMenu.Label
                                            >
                                            ...
                                            <div class="grid grid-cols-1 gap-4 p-4 md:grid-cols-2">
                                                <div class="border-background/60 bg-foreground/5 rounded-lg border p-3">
                                                    <p class="text-xs tracking-wide uppercase opacity-70">{(item.war as APIClanWar).clan.name}</p>
                                                    <div class="mt-2 space-y-3 text-sm">
                                                        <div>
                                                            <p class="text-xs tracking-wide uppercase opacity-70">Town Halls</p>
                                                            <div class="mt-2 flex flex-wrap gap-2">
                                                                {#each our.th.levels as lvl}
                                                                    <span class="bg-background/60 rounded-md px-2 py-1"
                                                                        >TH{lvl}: <span class="font-semibold">{our.th.counts[lvl]}</span></span
                                                                    >
                                                                {/each}
                                                            </div>
                                                        </div>
                                                        <div class="grid grid-cols-2 gap-2">
                                                            <div class="bg-background/60 flex items-center justify-between rounded-md px-3 py-1.5">
                                                                <span>Attacks</span><span class="font-semibold"
                                                                    >{our.attacksUsed}/{our.attacksAvailable}</span
                                                                >
                                                            </div>
                                                            <div class="bg-background/60 flex items-center justify-between rounded-md px-3 py-1.5">
                                                                <span>Left</span><span class="font-semibold">{our.attacksLeft}</span>
                                                            </div>
                                                        </div>
                                                        <div class="bg-background/60 flex items-center justify-between rounded-md px-3 py-1.5">
                                                            <span>Stars</span><span class="font-semibold">{our.stars} ⭐</span>
                                                        </div>
                                                    </div>
                                                </div>

                                                <div class="border-background/60 bg-foreground/5 rounded-lg border p-3">
                                                    <p class="text-xs tracking-wide uppercase opacity-70">{(item.war as APIClanWar).opponent.name}</p>
                                                    <div class="mt-2 space-y-3 text-sm">
                                                        <div>
                                                            <p class="text-xs tracking-wide uppercase opacity-70">Town Halls</p>
                                                            <div class="mt-2 flex flex-wrap gap-2">
                                                                {#each opp.th.levels as lvl}
                                                                    <span class="bg-background/60 rounded-md px-2 py-1"
                                                                        >TH{lvl}: <span class="font-semibold">{opp.th.counts[lvl]}</span></span
                                                                    >
                                                                {/each}
                                                            </div>
                                                        </div>
                                                        <div class="grid grid-cols-2 gap-2">
                                                            <div class="bg-background/60 flex items-center justify-between rounded-md px-3 py-1.5">
                                                                <span>Attacks</span><span class="font-semibold"
                                                                    >{opp.attacksUsed}/{opp.attacksAvailable}</span
                                                                >
                                                            </div>
                                                            <div class="bg-background/60 flex items-center justify-between rounded-md px-3 py-1.5">
                                                                <span>Left</span><span class="font-semibold">{opp.attacksLeft}</span>
                                                            </div>
                                                        </div>
                                                        <div class="bg-background/60 flex items-center justify-between rounded-md px-3 py-1.5">
                                                            <span>Stars</span><span class="font-semibold">{opp.stars} ⭐</span>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
