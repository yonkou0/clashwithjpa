<script lang="ts">
    ...
    function getClass(pathname: string, href: string | null): string {
        const baseClass =
            "relative text-base after:absolute after:bottom-0 after:left-0 after:h-[2px] after:w-full after:bg-foreground after:transition-transform after:duration-300 after:ease-in-out";
...
<svelte:element
    ...
    class={cn(
        ...
        "group font-coc flex items-center space-x-1 text-sm transition-all duration-300 ease-in-out",
        className
    )}
    {onclick}
